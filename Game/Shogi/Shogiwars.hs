...
downloadKifs :: ByteString -> IO ()
downloadKifs user = do
  urls <- kifUrls user
  forM_ (zip urls [0..]) $ \(u, i) -> do
    kif <- simpleHttp u
    print kif
    BL8.writeFile ((B8.unpack user) ++ show i) kif
  return ()


-- | userIdを受け取ってkifファイルの URLのリストを返す
kifUrls :: ByteString -> IO [String]
kifUrls user = do
--   c <- simpleHttp url
...
--   let params2 = map ((\(a,b) -> (B8.pack a, B8.pack b)) . (\(k,v) -> if k == "name1" then (k,user) else (k,v))) $ params
  let params2 :: [(B8.ByteString, B8.ByteString)]
      params2 = [("name1", user), ("csrfmiddlewaretoken", B8.pack $ fromJust $ lookup "csrfmiddlewaretoken" params), ("gtype", "0")]
...
--     cookie :: Cookie
    cookie params = Cookie { cookie_name = "csrftoken"
                     , cookie_value = fromJust $ lookup "csrfmiddlewaretoken" params
                     , cookie_path = "/"
